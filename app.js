...
// ---------- state ----------
const state = { user: null, profile: null, profiles: [], view: "dashboard", leadView: "kanban", dealView: "kanban", range: "30d", activitiesOk: true };
...
function renderAuth(mode = "signin") {
  ...
  <div class="auth-wrap"><div class="auth-card">
    ...
    <div class="muted" style="text-align:center;margin-top:16px;font-size:11px;letter-spacing:.05em">build 5</div>
  </div></div>`;
...
async function renderDashboard() {
  ...
  const [{ data: leads }, { data: deals }] = await Promise.all([
    supabase.from("leads").select("status,owner_id,created_at,updated_at,last_contact,name,tier"),
    supabase.from("deals").select("stage,tier,amount,owner_id,created_at,updated_at,closed_at,contact_name"),
  ]);
...
async function renderLeads() {
  ...
  if (state.leadView === "table") {
    ...
    main.insertAdjacentHTML("beforeend", `<div class="kanban">${LEAD_STATUS.map(([k, label]) => {
      ...
      return `<div class="col" data-col><div class="col-head"><span class="t">${label}</span><span class="c">${items.length}</span></div>
...
function leadCard(l) {
  return `<div class="card" data-edit="${l.id}" data-name="${esc((l.name || "").toLowerCase())}" data-tier="${l.tier || ""}">
...
function wireLeadCards(rows) {
  ...
  main.querySelectorAll("[data-move]").forEach((s) => (s.onchange = async () => { await supabase.from("leads").update({ status: s.value }).eq("id", s.dataset.move); renderLeads(); }));
...
function leadModal(lead = null) {
  ...
  `, [
    ...
    { label: editing ? "Save" : "Create", cls: "", act: async () => {
      const payload = { name: val("l_name"), tier: val("l_tier"), platform: val("l_platform"), contact_link: val("l_link"), status: val("l_status"), last_contact: val("l_last") || null, next_action: val("l_next"), notes: val("l_notes"), affordability_ok: document.getElementById("l_afford").checked };
...
async function convertLead(lead) {
  ...
  const { error: dErr } = await supabase.from("deals").insert({ contact_name: lead.name, tier: lead.tier, amount: tierAmount(lead.tier), stage: "call_booked", sourcing_setter: lead.owner_id, lead_id: lead.id });
...
async function renderDeals() {
  ...
  if (state.dealView === "table") {
    ...
    main.insertAdjacentHTML("beforeend", `<div class="kanban">${DEAL_STAGE.map(([k, label]) => {
      ...
      return `<div class="col" data-col><div class="col-head"><span class="t">${label}</span><span class="c">${items.length} · ${money(sum)}</span></div>
...
function dealCard(d) {
  return `<div class="card" data-edit="${d.id}" data-name="${esc((d.contact_name || "").toLowerCase())}" data-tier="${d.tier || ""}">
...
function wireDealCards(rows) {
  ...
  main.querySelectorAll("[data-move]").forEach((s) => (s.onchange = async () => {
    const id = s.dataset.move, prev = rows.find((r) => r.id === id);
    if (s.value === WON && prev?.stage !== WON) { const d = { ...prev, stage: WON }; await markClosed(d); }
    else { await supabase.from("deals").update({ stage: s.value }).eq("id", id); renderDeals(); }
  }));
}
...
function dealModal(deal = null) {
  ...
  `, [
    ...
    { label: editing ? "Save" : "Create", cls: "", act: async () => {
      ...
      const payload = { contact_name: val("d_name"), tier: val("d_tier"), amount: val("d_amount") ? +val("d_amount") : null, stage, owner_id: val("d_owner"), sourcing_setter: val("d_setter") || null, notes: val("d_notes"), stripe_sent: document.getElementById("d_stripe").checked, ayden_notified: document.getElementById("d_ayden").checked };
...
async function renderReports() {
  ...
  const [{ data: leads }, { data: deals }, actRes] = await Promise.all([
    supabase.from("leads").select("owner_id,status,tier,created_at,updated_at"),
    supabase.from("deals").select("owner_id,sourcing_setter,stage,tier,amount,created_at,updated_at,closed_at"),
